...
from cases import windmi


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run WINDMI with selectable L/C/Sigma mode and I_c trigger mode."
        )
    )
    ...
    parser.add_argument(
        "--start",
        required=True,
        help="Start datetime, for example 2000-12-30T00:00:00",
    )
    parser.add_argument(
        "--stop",
        required=True,
        help="Stop datetime, for example 2001-01-05T00:00:00",
    )
    parser.add_argument(
        "--mode-LCS",
        required=True,
        choices=["constant", "variable"],
        help="Set L, C, Sigma as constant or variable.",
    )
    parser.add_argument(
        "--mode-Ic",
        required=True,
        choices=["daily", "rolling"],
        help="Set I_c trigger mode as daily or rolling 3-hour window.",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
    ...
    parser.add_argument(
        "--ace-url-template",
        default=None,
        help="Optional download template for missing ACE files.",
    )
    parser.add_argument(
        "--no-prompt",
        action="store_true",
...
def main() -> None:
    ...
    if args.output_dir is None:
        ...
        output_dir = (
            Path("outputs")
            / f"windmi_{args.mode_LCS}_LCS_{args.mode_Ic}_Ic_{start_str}_to_{stop_str}"
        )
    else:
    ...
    out_dir = windmi(
        ...
        mode_LCS=args.mode_LCS,
        mode_Ic=args.mode_Ic,
